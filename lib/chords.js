...
function getEnharmonicEquivalent(chordText) {
    var idx = notes.indexOf(chordText.substring(0, 2)) - 1;

    if (idx < 0) {
        idx = notes.length - 1;
    }

    return notes[idx] + '#' + chordText.substring(2);
}
...
Chord.prototype.getText = function () {
    ...
    if (chordText.length > 1 && chordText.charAt(1) === 'b' && sharps.indexOf(this.transposeProperties.targetKey) > -1) {
        chordText = getEnharmonicEquivalent(chordText);
    }
